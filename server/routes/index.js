...
var makeIssue = function () {
    ...
    if(date.getTime()>first_issue_date.getTime()&&date.getTime()<end_issue_date.getTime()){
      //  正常销售
        ...
        var end_date = new Date(cur_issue_date.getTime()+h*10*60*1000);
      ...
      // 今日销售已停止
        ...
        end_time = first_issue_date.getTime();//截止信息
        cur_issue = [first_issue_date.getFullYear(),('0'+(first_issue_date.getMonth()+1)).slice(-2),('0'+first_issue_date.getDate()).slice(-2),'01'].join('');//期号信息
    }
    var cur_date = new Date();
    // 8分钟销售两分钟开奖
    if(end_time - cur_date.getTime() < 2*60*1000){
        state = '开奖中';
    } else {
        state = '正在销售';
    }
